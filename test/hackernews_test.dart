import 'package:flutter_test/flutter_test.dart';
import 'package:hacker_news_app/models/hackernews.dart';
import 'package:http/http.dart' as http;
import 'dart:convert';


void main() {
  test("parses item.json", () {
    const hackernewsJson = """{
  "by" : "dhouston",
  "descendants" : 71,
  "id" : 8863,
  "kids" : [ 8952, 9224, 8917, 8884, 8887, 8943, 8869, 8958, 9005, 9671, 8940, 9067, 8908, 9055, 8865, 8881, 8872, 8873, 8955, 10403, 8903, 8928, 9125, 8998, 8901, 8902, 8907, 8894, 8878, 8870, 8980, 8934, 8876 ],
  "score" : 111,
  "time" : 1175714200,
  "title" : "My YC app: Dropbox - Throw away your USB drive",
  "type" : "story",
  "url" : "http://www.getdropbox.com/u/2/screencast.html"
}""";
    expect(parseHackerNews(hackernewsJson).by, "dhouston");
  });

  test("parses item.json over the network", () async{
    const url ="https://hacker-news.firebaseio.com/v0/item/20050024.json";
    final response = await http.get(url);
    if (response.statusCode == 200){
      expect(parseHackerNews(response.body).by, 'dosy');
    }
  });

  test("json decode test", ()async{
    final String url = "https://hacker-news.firebaseio.com/v0/topstories.json";
    final  response =await http.get(url);
    print( jsonDecode(response.body));

  });

}
